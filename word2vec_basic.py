...
def generate_batch(data, batch_size, num_skips, skip_window):
  ...
  frame = (2 * skip_window) + 1
  b_queue = collections.deque(maxlen=frame)
  data_length = len(data)

  i = 0
  while i < frame:
    b_queue.append(data[data_index])
    next_index = (data_index + 1) % data_length
    data_index = next_index
    i += 1

  i = 0
  cnt = batch_size // num_skips

  while i < cnt:
    current = skip_window
    skip_current = [skip_window]

    j = 0
    while j < num_skips:
      rand_index = random.randint(0, frame - 1)
      skip_current.append(rand_index)
      batch[i * num_skips + j] = b_queue[skip_window]
      labels[i * num_skips + j, 0] = b_queue[rand_index]
      j += 1
